...
test.describe('Login success', () => {
  ...
  test('@TC1 login success when inputing valid username and password', async ({ page }) => {
    await loginPage.login(account.adminAccount.username, account.adminAccount.password)
  ...
  test('login success when inputing valid username and password and press enter key on password input', async ({ page }) => {
    await loginPage.login(account.adminAccount.username, account.adminAccount.password, false)
...
test.describe('Login failed', () => {
  ...
  test('login failed when inputing incorrect username and password', async ({ page }) => {
    await loginPage.login(account.invalidAccount.username, account.invalidAccount.password);
  ...
  test('login failed when inputing invalid password', async ({ page }) => {
    await loginPage.login(account.adminAccount.username, account.invalidAccount.password);
    await expect(loginPage.alertLocator).toBeVisible({
      timeout: 10000
    });
    await expect(loginPage.alertLocator).toHaveText('Invalid credentials');
  ...
  test('login failed when inputing invalid username', async ({ page }) => {
    await loginPage.login(account.invalidAccount.username, account.adminAccount.password);
    await expect(loginPage.alertLocator).toBeVisible({
      timeout: 10000
    });
    await expect(loginPage.alertLocator).toHaveText('Invalid credentials');
